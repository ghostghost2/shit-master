...
====================================
This guide will show you how to build shitd (headless client) for OSX.
...
5.  (Optional) You can also install shitd to your path:
...
------------------------
You can ignore this section if you are building `shitd` for your own use.

shitd/shit-cli binaries are not included in the shit-Qt.app bundle.

If you are building `shitd` or `shit-qt` for others, your build machine should be set up
...
Once dependencies are compiled, see release-process.md for how the Shit-Qt.app
bundle is packaged and signed to create the .dmg disk image that is distributed.

Running
-------

It's now available at `./shitd`, provided that you are still in the `src`
directory. We have to first create the RPC configuration file, though.

Run `./shitd` to get the filename where it should be put, or just try these
commands:
...
-------
    ...
    ./shitd -daemon # to start the shit daemon.
    ./shit-cli --help  # for a list of command-line options.
    ./shit-cli help    # When the daemon is running, to get a list of RPC commands
...
* Database errors have been seen in builds on High Sierra. One solution is to build Berkeley DB from source.
        ...
        Then configure Shit with this build of BerkeleyDB,
...
The use homebrew to install a number of unix programs and libraries needed to build the Shit wallet:
...
You have the choice to build the GUI Shit wallet as a Mac OSX app, described in “How to build the Shit-Qt App”. If, for whatever reason, you prefer to use the command line tools, continue with “Command line tools”.

### How to build the Shit-Qt App:

After make is finished, you can create an App bundle inside a disk image with:

```make deploy```

Once this is done, you’ll find Shit-Qt.dmg inside your Shit folder. Open and install the wallet like any typical Mac app.
