                                                          ...
                                                          currentBalance(-1)
{
    ...
    ui->zSHITpayAmount->setAttribute(Qt::WA_MacShowFocusRect, 0);
    

    // "Spending 999999 zSHIT ought to be enough for anybody." - Bill Gates, 2017
    ui->zSHITpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzSHITSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zSHITpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzSHIT_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_23_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSHIT is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zSHIT...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zSHIT in ") +
...
void PrivacyDialog::on_pushButtonSpendzSHIT_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_23_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSHIT is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zSHIT
        sendzSHIT();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zSHIT
    sendzSHIT();
}

void PrivacyDialog::on_pushButtonZShitControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ZShitControlDialog* zShitControl = new ZShitControlDialog(this);
    zShitControl->setModel(walletModel);
    zShitControl->exec();
}

void PrivacyDialog::setZShitControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzSHITSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzSHIT()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!IsValidDestinationString(ui->payTo->text().toStdString())) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Shit Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zSHITpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zSHIT is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " SHIT </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zSHIT</b>";
    ...
    // use mints from zSHIT selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZShitControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZShitControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zSHIT require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zSHIT"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZSHIT_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zSHIT require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zSHIT"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zSHIT was spent successfully update the addressbook with the label
    ...
    // Clear zshit selector in case it was used
    ZShitControlDialog::setSelectedMints.clear();
    ui->labelzSHITSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zSHIT Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Shit, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zSHIT Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zshitTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zSHIT </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zSHIT "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zSHIT "));
    ui->labelzSHITAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzSHITSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK23Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzSHIT->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zSHIT
        ui->pushButtonMintzSHIT->setEnabled(false);
        ui->pushButtonMintzSHIT->setToolTip(tr("zSHIT is currently disabled due to maintenance."));

        // Spend zSHIT
        ui->pushButtonSpendzSHIT->setEnabled(false);
        ui->pushButtonSpendzSHIT->setToolTip(tr("zSHIT is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zSHIT
        ui->pushButtonMintzSHIT->setEnabled(true);
        ui->pushButtonMintzSHIT->setToolTip(tr("PrivacyDialog", "Enter an amount of SHIT to convert to zSHIT", 0));

        // Spend zSHIT
        ui->pushButtonSpendzSHIT->setEnabled(true);
        ui->pushButtonSpendzSHIT->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}