...
#include "ui_zshitcontroldialog.h"
...
std::set<std::string> ZShitControlDialog::setSelectedMints;
std::set<CMintMeta> ZShitControlDialog::setMints;

ZShitControlDialog::ZShitControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZShitControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZShitControlDialog::~ZShitControlDialog()
{
    delete ui;
}

void ZShitControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZShitControlDialog::updateList()
{
...
void ZShitControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZShitControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZShit_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZShitControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZShitControlDialog::GetSelectedMints()
{
...
void ZShitControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
