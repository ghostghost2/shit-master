...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::SHIT);
    ...
    if (!settings.contains("nAnonymizeShitAmount"))
        settings.setValue("nAnonymizeShitAmount", 1000);

    nAnonymizeShitAmount = settings.value("nAnonymizeShitAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeShitAmount"))
        SoftSetArg("-anonymizeshitamount", settings.value("nAnonymizeShitAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in shit.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeShitAmount:
            return QVariant(nAnonymizeShitAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeShitAmount:
            nAnonymizeShitAmount = value.toInt();
            settings.setValue("nAnonymizeShitAmount", nAnonymizeShitAmount);
            emit anonymizeShitAmountChanged(nAnonymizeShitAmount);
            break;
        case CoinControlFeatures:
