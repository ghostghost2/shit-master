...
public:
    ...
    bool WriteZSHITSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSHITSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSHITSeed_deprecated(uint256& seed);
    bool EraseZSHITSeed();
    bool EraseZSHITSeed_deprecated();

    bool WriteZSHITCount(const uint32_t& nCount);
    bool ReadZSHITCount(uint32_t& nCount);
