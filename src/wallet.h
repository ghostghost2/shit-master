...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 SHIT at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zSHIT send
enum ZerocoinSpendStatus {
    ZSHIT_SPEND_OKAY = 0,                            // No error
    ZSHIT_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZSHIT_WALLET_LOCKED = 2,                         // Wallet was locked
    ZSHIT_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZSHIT_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZSHIT_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZSHIT_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZSHIT_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZSHIT_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZSHIT_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZSHIT_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZSHIT_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZSHIT_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZSHIT_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZSHIT_SPENT_USED_ZSHIT = 14,                      // Coin has already been spend
    ZSHIT_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZSHIT_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    void ZShitBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZSHITOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzshitAuto) const;
    ...
    */
    ...
    CzSHITWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzSHITTracker> zshitTracker;
    ...
    void setZWallet(CzSHITWallet* zwallet)
    {
        zwalletMain = zwallet;
        zshitTracker = std::unique_ptr<CzSHITTracker>(new CzSHITTracker(strWalletFile));
    }

    CzSHITWallet* getZWallet() { return zwalletMain; }
    ...
    void setZShitAutoBackups(bool fEnabled)
    {
