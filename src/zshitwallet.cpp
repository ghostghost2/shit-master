...
CzSHITWallet::CzSHITWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zshit seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSHITSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSHITSeed_deprecated()) {
                    LogPrintf("%s: Updated zSHIT seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zshit seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zshit wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSHITWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSHITCount(nCountLastUsed);
    else if (!walletdb.ReadZSHITCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSHITWallet::Lock()
{
    seedMaster = 0;
}

void CzSHITWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSHITWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSHIT(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSHITWallet::LoadMintPoolFromDB()
{
...
void CzSHITWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSHITWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSHITWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zshitTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSHITWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSHIT(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zshitTracker which also adds to database
    pwalletMain->zshitTracker->Add(dMint, true);
...
void CzSHITWallet::SeedToZSHIT(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSHITWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSHITWallet::GenerateDeterministicZSHIT(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSHITWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSHITWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
